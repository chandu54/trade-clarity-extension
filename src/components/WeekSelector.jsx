...
export default function WeekSelector({
  ...
  useEffect(() => {
    ...
    const listener = (req) => {
      if (req.action === "BULK_AI_PROGRESS") {
        ...
        req.action === "BULK_AI_ANALYSIS_COMPLETE" ||
        req.action === "BULK_AI_ANALYSIS_FAILED" ||
        req.action === "STOP_BULK_AI"
      ) {
        setIsBulkRunning(false);
      }
    };
  ...
  const handleBulkAnalyzeClick = () => {
    setIsBulkRunning(true);
    if (onBulkAnalyze) onBulkAnalyze();
  };
    ...
    <div className="week-selector">
      <div className="week-left">
        ...
        {(() => {
          ...
          const isBulkDisabled = isAiBlocked || isBulkRunning;
          return (
            <>
              <button
